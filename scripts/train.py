...
class TrainModel:
    ...
    def _define_feature_sets(self):
        self.GROWTH = [
            g
            for g in self.transformed_df
            if (g.find("growth_") == 0) & (g.find("future") < 0)
        ]
        self.OHLCV = ["Open", "High", "Low", "Close", "Adj Close", "Volume"]
        self.CATEGORICAL = ["Month", "Weekday", "Ticker", "ticker_type"]
        self.TO_PREDICT = [
            g for g in self.transformed_df.keys() if (g.find("future") >= 0)
        ]
        self.MACRO = [
            "gdppot_us_yoy",
            "gdppot_us_qoq",
        ...
        self.TECHNICAL_PATTERNS = [
            g for g in self.transformed_df.keys() if g.find("cdl") >= 0
        ]
        ...
        self.OTHER = [
            k
            for k in self.transformed_df.keys()
            if k
            not in self.OHLCV
            + self.CATEGORICAL
            + self.NUMERICAL
    ...
    def _define_dummies(self):
        ...
        dummy_variables = pd.get_dummies(
            self.transformed_df[self.CATEGORICAL], dtype="int32"
        )
    ...
    def _perform_temporal_split(
        ...
        train_end = min_date + pd.Timedelta(
            days=(max_date - min_date).days * train_prop
        )
    ...
    def _define_dataframes_for_ML(self):
        ...
        self.valid_df = self.df_full[self.df_full.split.isin(["validation"])].copy(
            deep=True
        )
        self.train_valid_df = self.df_full[
            self.df_full.split.isin(["train", "validation"])
        ].copy(deep=True)
        ...
        print(
            f"length: X_train {self.X_train.shape},  X_validation {self.X_valid.shape}, X_test {self.X_test.shape}"
        )
        print(
            f"  X_train_valid = {self.X_train_valid.shape},  all combined: X_all {self.X_all.shape}"
        )
    ...
    def prepare_dataframe(self):
        ...
        self._perform_temporal_split(
            self.df_full, min_date=min_date_df, max_date=max_date_df
        )
    ...
    def make_inference(self, pred_name: str):
        ...
        self.df_full[f"{pred_name}_rank"] = self.df_full.groupby("Date")[
            pred_name
