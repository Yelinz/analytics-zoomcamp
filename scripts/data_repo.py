...
class DataRepository:
    ...
    def _get_growth_df(self, df: pd.DataFrame, prefix: str) -> pd.DataFrame:
        ...
        for i in [1, 3, 7, 30, 90, 365]:
            df["growth_" + prefix + "_" + str(i) + "d"] = df["Adj Close"] / df[
                "Adj Close"
            ].shift(i)
    ...
    def fetch_tickers(self, min_date=None):
        ...
        for i, ticker in enumerate(tq):
            ...
            elif ticker in CAP_GAIN_STOCKS:
                historyPrices["ticker_type"] = "CAP_GAIN"
            else:
            ...
            historyPrices["high_minus_low_relative"] = (
                historyPrices.High - historyPrices.Low
            ) / historyPrices["Adj Close"]

            # 30d rolling volatility : https://ycharts.com/glossary/terms/rolling_vol_30
            historyPrices["volatility"] = historyPrices["Adj Close"].rolling(
                30
            ).std() * np.sqrt(252)

            # what we want to predict
            historyPrices["is_positive_growth_5d_future"] = np.where(
                historyPrices["growth_future_5d"] > 1, 1, 0
            )
            ...
            if self.ticker_df is None:
                ...
                self.ticker_df = pd.concat(
                    [self.ticker_df, historyPrices], ignore_index=True
                )
    ...
    def fetch_indexes(self, min_date=None):
        ...
        nikkei_daily_to_merge = self._get_growth_df(nikkei_daily, "nikkei225")
        vix_to_merge = vix.rename(columns={"Adj Close": "vix_adj_close"})[
            ["vix_adj_close"]
        ]
