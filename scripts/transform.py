...
class TransformData:
    ...
    def transform(self):
        ...
        self.transformed_df = self.transformed_df[
            self.transformed_df.Date >= "2000-01-01"
        ]
    ...
    def _transform_tickers(self):
        ...
        for ticker in tickers:
            ...
            df_current_ticker_momentum_indicators = self._get_talib_momentum_indicators(
                current_ticker_df
            )
            df_current_ticker_volume_indicators = (
                self._get_talib_volatility_cycle_price_indicators(current_ticker_df)
            )
            df_current_ticker_pattern_indicators = self._get_talib_pattern_indicators(
                current_ticker_df
            )
            ...
            current_ticker_df["Date"] = pd.to_datetime(
                current_ticker_df["Date"], utc=True
            )
            df_current_ticker_momentum_indicators["Date"] = pd.to_datetime(
                df_current_ticker_momentum_indicators["Date"], utc=True
            )
            df_current_ticker_volume_indicators["Date"] = pd.to_datetime(
                df_current_ticker_volume_indicators["Date"], utc=True
            )
            df_current_ticker_pattern_indicators["Date"] = pd.to_datetime(
                df_current_ticker_pattern_indicators["Date"], utc=True
            )

            # merge to one df
            m1 = pd.merge(
                current_ticker_df,
                df_current_ticker_momentum_indicators.reset_index(),
                how="left",
                on=["Date", "Ticker"],
                validate="one_to_one",
            )
            m2 = pd.merge(
                m1,
                df_current_ticker_volume_indicators.reset_index(),
                how="left",
                on=["Date", "Ticker"],
                validate="one_to_one",
            )
            m3 = pd.merge(
                m2,
                df_current_ticker_pattern_indicators.reset_index(),
                how="left",
                on=["Date", "Ticker"],
                validate="one_to_one",
            )
    ...
    def _get_talib_momentum_indicators(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        talib_momentum_adx = talib.ADX(
            df.High.values, df.Low.values, df.Close.values, timeperiod=14
        )
        # ADXR - Average Directional Movement Index Rating
        talib_momentum_adxr = talib.ADXR(
            df.High.values, df.Low.values, df.Close.values, timeperiod=14
        )
        # APO - Absolute Price Oscillator
        talib_momentum_apo = talib.APO(
            df.Close.values, fastperiod=12, slowperiod=26, matype=0
        )
        # AROON - Aroon
        ...
        talib_momentum_aroonosc = talib.AROONOSC(
        ...
        talib_momentum_bop = talib.BOP(
            df.Open.values, df.High.values, df.Low.values, df.Close.values
        )
        # CCI - Commodity Channel Index
        talib_momentum_cci = talib.CCI(
            df.High.values, df.Low.values, df.Close.values, timeperiod=14
        )
        # CMO - Chande Momentum Oscillator
        talib_momentum_cmo = talib.CMO(df.Close.values, timeperiod=14)
        # DX - Directional Movement Index
        talib_momentum_dx = talib.DX(
            df.High.values, df.Low.values, df.Close.values, timeperiod=14
        )
        # MACD - Moving Average Convergence/Divergence
        talib_momentum_macd, talib_momentum_macdsignal, talib_momentum_macdhist = (
            talib.MACD(df.Close.values, fastperiod=12, slowperiod=26, signalperiod=9)
        )
        # MACDEXT - MACD with controllable MA type
        (
            talib_momentum_macd_ext,
            talib_momentum_macdsignal_ext,
            talib_momentum_macdhist_ext,
        ) = talib.MACDEXT(
            df.Close.values,
            fastperiod=12,
            fastmatype=0,
        ...
        (
            talib_momentum_macd_fix,
            talib_momentum_macdsignal_fix,
            talib_momentum_macdhist_fix,
        ) = talib.MACDFIX(df.Close.values, signalperiod=9)
        # MFI - Money Flow Index
        talib_momentum_mfi = talib.MFI(
            df.High.values,
            df.Low.values,
            df.Close.values,
            df.Volume.values,
        ...
        talib_momentum_minus_di = talib.MINUS_DM(
            df.High.values, df.Low.values, timeperiod=14
        )
        # MOM - Momentum
        talib_momentum_mom = talib.MOM(df.Close.values, timeperiod=10)
        # PLUS_DI - Plus Directional Indicator
        talib_momentum_plus_di = talib.PLUS_DI(
            df.High.values, df.Low.values, df.Close.values, timeperiod=14
        )
        # PLUS_DM - Plus Directional Movement
        talib_momentum_plus_dm = talib.PLUS_DM(
            df.High.values, df.Low.values, timeperiod=14
        )
        # PPO - Percentage Price Oscillator
        talib_momentum_ppo = talib.PPO(
            df.Close.values, fastperiod=12, slowperiod=26, matype=0
        )
        ...
        # WILLR - Williams' %R
        talib_momentum_willr = talib.WILLR(
            df.High.values, df.Low.values, df.Close.values, timeperiod=14
        )
    ...
    def _get_talib_volatility_cycle_price_indicators(
        ...
        talib_ht_sine_sine, talib_ht_sine_leadsine = talib.HT_SINE(df.Close.values)
    ...
    def persist(self, data_dir: str):
        ...
        if os.path.exists(file_name):
            os.remove(file_name)
        self.transformed_df.to_parquet(
            os.path.join(data_dir, file_name), compression="brotli"
        )

    def load(self, data_dir: str):
        """Load files from the local directory"""
        os.makedirs(data_dir, exist_ok=True)
        self.transformed_df = pd.read_parquet(
            os.path.join(data_dir, "transformed_df.parquet")
        )
